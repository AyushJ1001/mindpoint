...
for (const route of routes) {
  ...
  await page.screenshot({ path: `output/playwright/route-${key}-top-after.png`, fullPage: false });

  const scrollY = Math.min(2200, Math.max(0, report.docHeight - 844));
  await page.evaluate((y) => window.scrollTo(0, y), scrollY);
  await page.waitForTimeout(250);
  await page.screenshot({ path: `output/playwright/route-${key}-mid-after.png`, fullPage: false });
